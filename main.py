...
map_query = '''
SELECT rd."city_state", rd."state_name", rd."RegionID", rd."lat", rd."lng",
  AVG(COALESCE(rby."January", 0)) AS avg_january,
  AVG(COALESCE(rby."February", 0)) AS avg_february,
  AVG(COALESCE(rby."March", 0)) as avg_march,
  AVG(COALESCE(rby."April", 0)) as avg_april,
  AVG(COALESCE(rby."May", 0)) as avg_may,
  AVG(COALESCE(rby."June", 0)) as avg_june,
  AVG(COALESCE(rby."July", 0)) as avg_july,
  AVG(COALESCE(rby."August", 0)) as avg_august,
  AVG(COALESCE(rby."September", 0)) as avg_september,
  AVG(COALESCE(rby."October", 0)) as avg_october,
  AVG(COALESCE(rby."November", 0)) as avg_november,
  AVG(COALESCE(rby."December" , 0)) as avg_december
  FROM realestate_by_year rby
  LEFT OUTER JOIN realestatedata rd ON rby."RegionID" = rd."RegionID"
  WHERE rd."state_name" = %s
  GROUP BY rd."city_state", rd."state_name", rd."RegionID", rd."lat", rd."lng"
'''
map_data = pd.read_sql(map_query, engine, params=(state,))
map_df = pd.DataFrame(map_data)
map_df['latitude'] = map_df['lat']
map_df['longitude'] = map_df['lng']
city_state = map_df['state_name'].iloc[0]
st.subheader(f'Map of Average Home Values For {city_state} for January')
...
points = alt.Chart(map_df).mark_circle().encode(
    longitude='longitude:Q',
    latitude='latitude:Q',
    size=alt.value(10),
    tooltip='avg_january:Q',
    color=alt.Color('avg_january:Q', scale=alt.Scale(scheme='blues'))
)
